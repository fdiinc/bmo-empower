...
public class SearchResultPane extends AbstractSearchResultPane {
  ...
  private class SelectAllButton extends Button implements ClickHandler, DataChangeListener {
    ...
    SelectAllButton(TableSelection selection, StoredQueryDataSource dataSource) {
      this.selection = selection;
      this.dataSource = dataSource.cloneDataSource();
  ...
  private class ExportResultSetButton extends Button implements ClickHandler {
    ...
    @Override
    public void onClick(ClickEvent event) {
      LoadingDialog.show("export-result-set");
      StoredQueryDataSource src = getDataSource().cloneDataSource();
      ...
      logRequest(applicationSettings.getApplicationContext().getApplicationName(), applicationSettings.getState().getCurrentSearchSetting(), dataSource.getFields(),
              applicationSettings.getUserName(), applicationSettings.getRole().getUnauthorizedFields(), "Export-Results");
      runProxyExport(xQuery);
      ContentPane.userInfoService.onXProcComplete(new AsyncCallback<Void>() {

    	@Override
    	public void onSuccess(Void result) {
    		LoadingDialog.hide("export-result-set");
    	}
    ...
    private String embedExternalVariablesXQuery(StoredQueryDataSource src) {
      ...
      // This should really strip all the declares (not a hardcoded list) and replace each one with it's field, value pairing

      Map<String,String> searchFieldsMap = src.getFields();
      for (String key : searchFieldsMap.keySet()) {
        String currentDeclare = "declare variable $"+key+" as xs:string external;";
        xQuery = xQuery.replace(currentDeclare, "let $"+key+" := '"+searchFieldsMap.get(key)+"'");
      }
    ...
    private void runProxyExport(String fullXQuery) {
         ...
         String styleSheetURI = "dds://DOMAIN=resource/xslt/exportExcelCSV.xsl";
         //String pipelineURI = "dds://DOMAIN=resource/xproc/displayxqueryresult.xpl";
         String pipelineURI = "dds://DOMAIN=resource/xproc/CSVDisplayxqueryresult.xpl";
         String contentType = "application/vnd.ms-excel";
         // String contentType = "text/xml";
         boolean useExternalOutput = true;

         Map<String, String> headers = new HashMap<String, String>(2);
         //change for csv output
         // headers.put("Content-Disposition", "attachment; filename=ExcelExport.xml");
         headers.put("Content-Disposition", "attachment; filename=ExcelExport.csv");
         ...
         String url = requestSerializer.getRequestString(
                 pipelineURI, input, contentType, useExternalOutput, headers);
  ...
  public static final String CORE_HEIGHT = "400px";
  ...
  protected int loadTable(List<SerializableXQueryValue> results) {
    ...
    if (decorator != null) {
      ...
      // Reload the pager
      addFullFooter();
      getPagingBar().selectPage(getPagingBar().getSelectedPage());
    }
    return numItems;
  }

  protected void addFullFooter() {
...
//      }
  ...
  public void handle(ApplicationEvent event) {
    ...
    switch (event.getType()) {
      ...
      case ApplicationEvent.NESTED_SEARCH_SUBMIT_EVENT:
    	  if(getContentViewPane().isVisible()) {
              getContentViewPane().clearContentViewFooter();
          }
        starttime = System.currentTimeMillis();
        addFullFooter();
        break;
      case ApplicationEvent.SEARCH_CLEAR_EVENT:
        decorator.setTopText(HEADER_TEXT, false);
        HorizontalPanel hPanel2 = new HorizontalPanel();
        hPanel2.setVerticalAlignment(HasVerticalAlignment.ALIGN_MIDDLE);
        hPanel2.setSize("100%", "100%");
        hPanel2.add(getPagingBar());
        clearItems();
        decorator.setFooterWidget(hPanel2);
        break;
      case ApplicationEvent.DETAILS_PANE_CLEAR_EVENT:

          if (getContentViewPane().isVisible()) {
              getContentViewPane().clearContentViewPane();
              getContentViewPane().clearContentViewFooter();
          }
          break;
    }
  ...
  private void displaySelectionResult(OperationConfiguration operationConfiguration,
    ...
    if (getContentViewPane().isAttached()) {
